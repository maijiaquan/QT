...
int MainWindow::isBodyAhead(){
    ...
    /*遍历数组*/
    for(int i = 0; i < n; i++){
        bool isRight = head->y == p[i][1] && head->x + 1 == p[i][0] && snake_dir == DIR_RIGHT;
        bool isLeft = head->y == p[i][1] && head->x - 1 == p[i][0] && snake_dir == DIR_LEFT;
        bool isUp = head->x == p[i][0] && head->y - 1 == p[i][1] && snake_dir== DIR_UP;
        bool isDown = head->x == p[i][0] && head->y + 1 == p[i][1] && snake_dir== DIR_DOWN;
        if(isRight){
            return DIR_RIGHT;
        }
        else if(isLeft){
